...
namespace TUIO {
	...
	public:
		...
		int getScreenDistance(int xp, int yp, int w, int h) {
			float dx = floor(xpos*(float)w +0.5f)-(float)xp;
			float dy = floor(ypos*(float)h +0.5f)-(float)yp;
			return (int)sqrtf(dx*dx+dy*dy);
		}

		int getScreenDistance(TuioPoint tpoint, int w, int h){
			return getScreenDistance(tpoint.getScreenX(w),tpoint.getScreenY(h), w, h);
		}
