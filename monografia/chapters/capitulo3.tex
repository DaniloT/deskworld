...
Conforme mencionado no capítulo anterior, existem dois principais métodos de iluminação de uma mesa multitoque, \textit{FTIR}(\textit{Frustrated Total Internal Refraction}) e \textit{DI}(\textit{Diffuse Illumination}). \textit{FTIR} trata-se de uma iluminação em que \textit{LEDs}(textit{Light-Emitting Diode}) são postos em paralelo a superfície de acrílico da mesa, ou dentro do acrílico, de modo que os feixes de luz fiquem presos em seu interior, sendo refletidos internamente devido ao fenômeno de reflexão interna total~[\citenum{TIR}].
